...
const ConditionalTooltip: React.FC<{
    ...
    const tooltipContent = (
        <TooltipProvider>
            <Tooltip>
                <TooltipTrigger asChild>{React.cloneElement(children, { ref: textRef })}</TooltipTrigger>
            ...
            <div className="h-full flex flex-col border-l dark:border-slate-700 bg-white dark:bg-card overflow-hidden">
                ...
                <div className="flex-1 flex items-center justify-center p-4">
                    <div className="text-center">
                        ...
                        <p className="text-gray-500 dark:text-gray-400">Select an element from the tree or list view to see its properties</p>
                    </div>
                </div>
            </div>
        );
            ...
            <div className="flex-1 overflow-y-auto p-4">
                {object.properties ? (
                    Object.keys(object.properties).length > 0 ? (
                        <div>
                            {Object.entries(object.properties).map(([category, props]) => (
                                <div key={category} className="mb-4">
                                    {/* Category Header */}
                                    <h3 className="font-semibold text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-neutral-800 p-2 rounded">{category}</h3>
                                    ...
                                    <div className="border dark:border-slate-700 rounded">
                                        {Object.entries(props).map(([propName, propValue]) => (
                                            <div key={propName} className="flex border-b last:border-b-0 dark:border-neutral-600">
                                                ...
                                                <div className="w-2/5 py-1 px-4 bg-gray-50 dark:bg-neutral-700 text-gray-600 dark:text-gray-400">{String(propValue)}</div>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                        ...
                        // Show this message when the properties object exists but is empty
                        <p className="text-gray-500 dark:text-gray-400">This element has no properties</p>
                    )
                ) : (
                    // Fallback message when properties object is null or undefined
